    ...
    try {
        ...
        if (!isMO) {
            if (!activeRole || activeRole === "No Role Assigned") {
                ...
                // Role grants <AllComputers/> — trust BigFix's server-side RBAC (user's own creds already filter)
        ...
        if (
            ...
            await setWorkflowState(WORKFLOW_STATE.RUNNING, envLabel);
        }
        ...
        for (const dep of deployments) {
            ...
            if (actionId) {
                const metadata = {
                    ...
                    endOffset: endDateTimeOffsetVal, preMail: shouldMail, smtpEnabled: smtpReady, postMailSent: false, triggeredBy: triggeredBy || "Unknown"
                };
        ...
        if (shouldMail && smtpReady && generatedActions.length > 0) {
            try {
                ...
                await sendTriggerMail(ctx.smtp, { environment: envLabel, baselineName: "Multiple Baselines (See Attached CSV)", baselineSite: "Multiple Sites", baselineFixletId: "Multiple IDs", groupName: "Multiple Groups (See Attached CSV)", groupId: "Multiple IDs", groupSite: "Multiple Sites", groupType: "Multiple Types", actionId: generatedActions.map(a => a.actionId).join(", "), endOffset: endDateTimeOffsetVal, emailTo: mailTo, emailFrom: mailFrom, emailCc: mailCc, emailBcc: mailBcc, SMTP_FROM, SMTP_TO, SMTP_CC, SMTP_BCC, csvContent, patchesCsvContent, baselinePatches: [] });
            } catch (e) { emailError = e.message || String(e); }
        }


        // Persist team-state for the active role atomically with the trigger response so the
        // orchestration UI survives an immediate page refresh. The frontend's saveState() is
        // best-effort; this server-side write is authoritative.
        if (generatedActions.length > 0 && envLabel.toLowerCase() === 'sandbox') {
            ...
            if (roleToWrite) {
                try {
                    await setTeamStateForRole(roleToWrite, {
                        ...
                        lastActions: { SANDBOX: { actions: generatedActions, ts: Date.now() } }
                    });
                } catch (e) { /* helper already logs */ }
            }
        }

        return res.json({ ok: true, actions: generatedActions, stage: envLabel, endOffset: endDateTimeOffsetVal, createdAt: new Date().toISOString(), preMail: shouldMail, preMailError: emailError });

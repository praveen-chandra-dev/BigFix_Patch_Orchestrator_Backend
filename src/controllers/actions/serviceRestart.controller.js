...
async function serviceRestart(req, res) {
  ...
  const cleanComputerName = String(computerName).replace(/[\r\n]/g, "");

  const ctx = getCtx();
  const { BIGFIX_BASE_URL } = ctx.bigfix;

  try {
    const safeComputerName = cleanComputerName.toLowerCase().replace(/"/g, '\\"');
    const relevance = `(ids of it) of bes computers whose (name of it as lowercase = "${safeComputerName}")`;
    const rawUrl = `${joinUrl(BIGFIX_BASE_URL, "/api/query")}?output=json&relevance=${encodeURIComponent(relevance)}`;
    
    
    const safeUrl = new URL(rawUrl);

    const bfAuthOpts = await getBfAuthContext(req, ctx); 
    const resp = await axios.get(safeUrl.toString(), { ...bfAuthOpts, headers: { Accept: "application/json" } });
    ...
    res.json({ ok: true, actionId: extractActionIdFromXml(String(bfResp.data || "")), computerId, computerName: cleanComputerName });

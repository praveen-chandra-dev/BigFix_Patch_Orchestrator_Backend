...
const { setTeamStateForRole } = require("../team.controller");
...
async function handlePilotTrigger(req, res, isForced) {
    const ctx = getCtx();
    ...
    log(req, `POST /api/pilot/actions${isForced ? '/force' : ''}. User: [${triggeredBy || 'Unknown'}].`);
    ...
    try {
        ...
        // FIX: Removed timezone subtraction to use an absolute, global UTC duration
        const endDateTimeOffsetVal = msToXSDuration(pwMs);
        ...
        let compFilter = "";
        if (!isMO) {
            if (!activeRole || activeRole === "No Role Assigned") return res.status(403).json({ ok: false, error: "No role assigned. Cannot trigger action." });
                ...
                if (roleAssets.allComputers) {
                    compFilter = "";
                } else if (roleAssets.compNames && roleAssets.compNames.length > 0) {
        ...
        const currentState = await getWorkflowState();

        if (
            currentState.state === WORKFLOW_STATE.NEW ||
            currentState.state === WORKFLOW_STATE.RESET ||
            currentState.state === WORKFLOW_STATE.COMPLETED
        ) {
            await setWorkflowState(WORKFLOW_STATE.RUNNING, environment);
        }
        ...
        for (const dep of deployments) {
            ...
            if (actionId) {
                const metadata = { id: actionId, createdAt: new Date().toISOString(), expiresAt, stage: environment, xml, baselineName, baselineSite: siteName, baselineFixletId: fixletId, groupName: gName, groupId: gId, groupSite: gSite, groupType: gType, endOffset: endDateTimeOffsetVal, preMail: shouldMail, smtpEnabled: smtpReady, postMailSent: false, triggeredBy: triggeredBy || "Unknown" };
        ...
        if (shouldMail && smtpReady && generatedActions.length > 0) {
            try {
                ...
                await sendTriggerMail(ctx.smtp, { environment, baselineName: "Multiple Baselines (See Attached CSV)", baselineSite: "Multiple Sites", baselineFixletId: "Multiple IDs", groupName: "Multiple Groups (See Attached CSV)", groupId: "Multiple IDs", groupSite: "Multiple Sites", groupType: "Multiple Types", actionId: generatedActions.map(a => a.actionId).join(", "), endOffset: endDateTimeOffsetVal, emailTo: mailTo, emailFrom: mailFrom, emailCc: mailCc, emailBcc: mailBcc, SMTP_FROM: ctx.smtp.SMTP_FROM, SMTP_TO: ctx.smtp.SMTP_TO, SMTP_CC: ctx.smtp.SMTP_CC, SMTP_BCC: ctx.smtp.SMTP_BCC, csvContent, patchesCsvContent, baselinePatches: [] });
            } catch (e) { emailError = e.message; }
        }

        // Persist team-state for the active role BEFORE responding. This ensures the next
        // page load (from this user or any teammate) sees pilotTriggered=true and the new
        // action list — even if the frontend's saveState() never gets to run because the
        // user refreshed immediately.
        if (generatedActions.length > 0) {
            const roleToWrite = (activeRole && activeRole !== 'No Role Assigned')
                ? (isMO ? 'Admin' : activeRole)
                : null;
            if (roleToWrite) {
                try {
                    await setTeamStateForRole(roleToWrite, {
                        pilotTriggered: true,
                        runningStage: "PILOT",
                        lastActions: { PILOT: { actions: generatedActions, ts: Date.now() } }
                    });
                } catch (e) { /* helper already logs */ }
            }
        }

        res.json({ ok: true, chgOk: !requireChg || isForced || true, forced: isForced, actions: generatedActions, stage: environment, endOffset: endDateTimeOffsetVal, createdAt: new Date().toISOString(), preMailError: emailError });

    } catch (err) { res.status(500).json({ ok: false, error: String(err?.message || err) }); }
}

const triggerPilot = (req, res) => handlePilotTrigger(req, res, false);
const triggerPilotForce = (req, res) => handlePilotTrigger(req, res, true);

module.exports = { triggerPilot, triggerPilotForce };
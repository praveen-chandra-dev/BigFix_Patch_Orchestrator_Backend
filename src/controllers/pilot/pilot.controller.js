...
//         CONFIG.lastPilotBaseline = baselineName; CONFIG.lastPilotGroup = gName;
    ...
    try {
      ...
      if (shouldMail && smtpReady && generatedActions.length > 0) { 
          try {
            ...
            await sendTriggerMail(ctx.smtp, { environment, baselineName: "Multiple Baselines (See Attached CSV)", baselineSite: "Multiple Sites", baselineFixletId: "Multiple IDs", groupName: "Multiple Groups (See Attached CSV)", groupId: "Multiple IDs", groupSite: "Multiple Sites", groupType: "Multiple Types", actionId: generatedActions.map(a => a.actionId).join(", "), endOffset: endDateTimeLocalOffsetVal, emailTo: mailTo, emailFrom: mailFrom, emailCc: mailCc, emailBcc: mailBcc, SMTP_FROM: ctx.smtp.SMTP_FROM, SMTP_TO: ctx.smtp.SMTP_TO, SMTP_CC: ctx.smtp.SMTP_CC, SMTP_BCC: ctx.smtp.SMTP_BCC, csvContent, patchesCsvContent, baselinePatches: [] });
          } catch (e) { emailError = e.message; }
      }

      res.json({ ok: true, chgOk: !requireChg || isForced || true, forced: isForced, actions: generatedActions, stage: environment, endOffset: endDateTimeLocalOffsetVal, createdAt: new Date().toISOString(), preMailError: emailError });

...
async function handleProductionTrigger(req, res, isForced) {
    ...
    try {
        ...
        // Persist team-state for the active role atomically with the trigger response.
        // Same rationale as the Pilot controller — avoids the client-side race where a quick
        // refresh after triggering loses the stage's action list.
            ...
            if (roleToWrite) {
                try {
                    await setTeamStateForRole(roleToWrite, {
                        ...
                        runningStage: "PRODUCTION",
                        lastActions: { PRODUCTION: { actions: generatedActions, ts: Date.now() } }
                    });

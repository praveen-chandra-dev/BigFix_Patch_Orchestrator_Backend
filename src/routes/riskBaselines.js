...
//  BOLA FIX: Added RBAC checks to baseline location lookup to ensure the user is authorized to access the site.
async function getBaselineLocation(req, ctx, baselineId) {
  const bfAuthOpts = await getBfAuthContext(req, ctx); // Ensure user context is passed
  const bfUrl = (ctx.cfg?.BIGFIX_BASE_URL || "").replace(/\/$/, "");

  const activeUser = getSessionUser(req);
  const activeRole = getSessionRole(req)  /* Vuln 8 fix: role from session only */ || "Default";
  const isMO = await isMasterOperator(req, ctx, activeUser);
  
  let allowedSitesSet = null;
  if (!isMO) {
     const roleAssets = await getRoleAssets(req, ctx, activeRole);
     allowedSitesSet = new Set([...(roleAssets.customSites || []), ...(roleAssets.externalSites || [])].map(s => s.toLowerCase()));
  }
  ...
  const parseSiteInfo = (val) => {
    ...
    //  BOLA FIX: Validate site authorization
    if (!isMO) {
         if (!allowedSitesSet.has(siteName.toLowerCase())) {
              throw new Error(`Unauthorized access to baseline in site: ${siteName}`);
         }
    }
    return { siteType: siteType.trim(), siteName };
  };
...
function attachBaselineRoutes(app, ctx) {
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      if (!/^\d+$/.test(bfId)) return res.status(400).json({ error: "Invalid baseline ID" }); // Sanitize ID
  ...
  router.put("/:id", async (req, res) => {
    try {
        const bfId = req.params.id; 
        if (!/^\d+$/.test(bfId)) return res.status(400).json({ error: "Invalid baseline ID" });
 ...
 router.delete("/:id", async (req, res) => {
    try {
        ...
        try {
            await axios.delete(buildBaselineUrl(bfUrl, loc.siteType, loc.siteName, bfId), bfAuthOpts);
        } catch (bfErr) {
        ...
        try {
            ...
            // Silencing Prism deletion errors as it's secondary
        }

        res.json({ success: true });
    } catch (err) { 
        const errMsg = err.response?.data || err.message || "Deletion failed";
        const status = errMsg.includes("Unauthorized") ? 403 : (errMsg.includes("not found") ? 404 : 500);
        res.status(status).json({ ok: false, error: String(errMsg).replace(/"/g, "'") }); 
    }
  });
    ...
    try {
      ...
      if (requestedLimit > 1000) {
          return res.status(400).json({ 
              ok: false, 
              error: "Bad Request: 'limit' cannot exceed 1000." 
          });
      }

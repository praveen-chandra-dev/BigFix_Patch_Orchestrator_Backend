...
// We export the same attachment function so app.js requires ZERO changes
function attachActionsRoutes(app, ctx) {
  
  // Computer Restart endpoints
  app.post("/api/actions/restart", actionsController.restartSingle);
  app.post("/api/actions/restart-bulk", actionsController.restartBulk);
  
  // Windows Update Service Restart endpoint
  app.post("/api/actions/service-restart", actionsController.serviceRestart);
  
  // Primary Baseline Deployment (Sandbox) endpoint
  app.post("/api/actions", actionsController.triggerAction);
...
//       // 2) Group lookup (With DB Self-Healing check)
//       const qGroup = `(name of it, id of it, name of site of it, (if automatic flag of it then "Automatic" else if manual flag of it then "manual" else "server based")) of bes computer group whose (name of it is "${groupName.replace(/"/g, '\\"')}")`;
//       const urlGroup = `${joinUrl(BIGFIX_BASE_URL, "/api/query")}?output=json&relevance=${encodeURIComponent(qGroup)}`;
//       const groupResp = await axios.get(urlGroup, { ...bfAuthOpts, headers: { Accept: "application/json" }, validateStatus: () => true });
//       if (groupResp.status < 200 || groupResp.status >= 300) return res.status(groupResp.status).send(groupResp.data);
...
//       // 3) 🚀 NEW: Get specific Computer IDs for the target group
...
//           return res.status(404).json({ ok: false, error: `No computers found in group '${gName}' or you do not have permission to view them.` });
//       }
...
//       // 4) CSV & Patch Content Generation for Email
//       let csvContent = null;
//       let patchesCsvContent = null;
//       let baselinePatches = [];
...
//       // 5) 🚀 Calculate EndDateTimeLocalOffset
...
//       console.log(`[${envLabel}] Posting Action to BigFix with XML:`, xml);
...
//       return res.json({ 
//           ok: true, actionId, siteName, fixletId, group: gName, title: actionTitle, stage: envLabel, 
//           endOffset: endDateTimeLocalOffsetVal, createdAt: new Date().toISOString(), preMail: shouldMail, preMailError: emailError 
//       });
//     } catch (err) { return res.status(500).json({ ok: false, error: err?.response?.data || err.message }); }
//   }

...
/** Parse:  "Name | Id | State | Issued | Stopped" → {name,id,state,issued,stopped} */
function parseRow(s) {
...
function attachDeploymentsRoutes(app, ctx) {
  ...
  // GET /api/deployments/bps  → list actions that start with "BPS_"
  app.get("/api/deployments/bps", async (req, res) => {
    ...
    try {
      const relevance =
        ...
        `(time stopped of it as string | "N/A")) of bes actions whose (name of it starts with "BPS_")`;
      ...
      // result can be nested; flatten to strings
      const flat = [];
      const collect = (n) => {
        if (n == null) return;
      ...
      const items = flat
        .filter(Boolean)
        .map(parseRow)
        // newest id first if they’re numeric

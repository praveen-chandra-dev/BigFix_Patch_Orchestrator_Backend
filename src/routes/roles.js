...
function isAdmin(req) {
  // Vuln 8 fix: use server-validated session, not raw cookie
  return !!(req.session && req.session.Role && req.session.Role.toLowerCase() === 'admin');
}
...
const safeBool = (val) => (val === "1" || val === 1 || val === true || val === "true" ? "1" : "0");
const safeEnum = (val, allowed, defaultVal) => allowed.includes(val) ? val : defaultVal;
...
function buildRoleXml(data) {
    ...
    <BESAPI xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="BESAPI.xsd">
        <Role>
            ...
            <MasterOperator>${safeBool(data.perms.masterOperator)}</MasterOperator>
            <CustomContent>${safeBool(data.perms.customContent)}</CustomContent>
            <ShowOtherActions>${safeBool(data.perms.showOtherActions)}</ShowOtherActions>
            <StopOtherActions>${safeBool(data.perms.stopOtherActions)}</StopOtherActions>
            <CanCreateActions>${safeBool(data.perms.canCreateActions)}</CanCreateActions>
            <PostActionBehaviorPrivilege>${safeEnum(data.perms.postActionBehavior, ['None', 'AllowRestartOnly', 'AllowAll'], 'AllowRestartOnly')}</PostActionBehaviorPrivilege>
            <ActionScriptCommandsPrivilege>${safeEnum(data.perms.actionScriptCommands, ['None', 'AllowRestartOnly', 'AllowAll'], 'AllowRestartOnly')}</ActionScriptCommandsPrivilege>
            <CanSendMultipleRefresh>${safeBool(data.perms.canSendRefresh)}</CanSendMultipleRefresh>
            <CanSubmitQueries>${safeBool(data.perms.canSubmitQueries)}</CanSubmitQueries>
            <CanLock>${safeBool(data.perms.canLock)}</CanLock>
            <UnmanagedAssetPrivilege>${safeEnum(data.perms.unmanagedAssets, ['ShowNone', 'ShowAll'], 'ShowNone')}</UnmanagedAssetPrivilege>
            <InterfaceLogins>
                <Console>${safeBool(data.perms.useConsole) === "1" ? "true" : "false"}</Console>
                <WebUI>${safeBool(data.perms.useWebUI) === "1" ? "true" : "false"}</WebUI>
                <API>${safeBool(data.perms.useRESTAPI) === "1" ? "true" : "false"}</API>
...
router.post('/api/roles/create', async (req, res) => {
    ...
    try {
        ...
        const creator = req.session.Username;  // Vuln 8 fix;

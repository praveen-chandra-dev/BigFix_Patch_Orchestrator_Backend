...
const { isMasterOperator, bigfixClient, getUserVisibleComputers } = require("../services/bigfix");
...
function attachHealthRoutes(app, ctx) {
  ...
  app.get("/api/infra/total-computers", async (req, res) => {
    try {
      const activeRole = getSessionRole(req)  // Vuln 8 fix: role from server session only;
      ...
      if (!isMO) {
          if (!activeRole || activeRole === "No Role Assigned") {
              ...
              compList = compList.filter(c => allowedSet.has(String(c).toLowerCase().trim()));
          }
      }
    ...
    try {
      ...
      if (now - globalHealthCache.critical.lastFetch > CACHE_TTL || req.query.refresh === 'true') {
          ...
          if (resp.status === 200) {
              ...
              const parseDiskGB = (s) => {
                // FIX S5852: Replaced \d+ with \d{1,15} to bound quantifiers and prevent ReDoS
                const regex = /(\d{1,15}(?:\.\d{1,15})?)\s{0,5}GB/i;

...
const fileFormat = winston.format.combine(
  ...
  winston.format.splat(),
  winston.format((info) => {
    // FIX: Use dynamically generated regex to avoid literal control characters (SAST S6324)
    const colorRegex = new RegExp(String.fromCharCode(27) + '\\[[0-9;]*m', 'g');
    info.level = info.level.replace(colorRegex, ''); // strip colors
    return info;
  })(),
  besFormatter
);

...
function createEmailTemplate({ title, subtitle, details = [], csvAttached = false, statusColor = "#0078D4" }) {
  ...
  // FIX: Subtitle ko optional banaya gaya hai (agar empty string hai toh render nahi hoga)
...
async function sendTriggerMail(smtp, {
  ...
  // FIX: Subject line update ki gayi
  const subject = `Pre-Patching Triggered For Baseline ${baselineName}`;

  // FIX: Unnecessary fields hata diye gaye
  const details = [
    ...
    // { key: "Baseline Site", value: baselineSite },
    // { key: "Baseline FixletID", value: baselineFixletId },
    // { key: "Group ID", value: groupId || "N/A" },
    // { key: "Group Site", value: groupSite },
    // { key: "Patch Window", value: endOffset || "N/A" },
  ];

  const html = createEmailTemplate({
    title: `${stageName} Patching Triggered`,
    subtitle: "", // FIX: Subtitle hata diya gaya
    details: details,
...
async function sendPostPatchMail(smtp, {
  ...
  const details = [
    ...
    { key: "Action ID", value: actionId || "Unknown" },
    { key: "Baseline", value: baselineName },
    { key: "Target Group", value: groupName }, // Yeh ab 'postpatchWatcher' se sahi ayega

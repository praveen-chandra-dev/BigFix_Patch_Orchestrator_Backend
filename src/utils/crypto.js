...
function logKeyFingerprint(key) {
    ...
    if (fingerprint !== lastKeyFingerprint) {
        console.log('[Crypto] Master encryption key loaded successfully.');
...
function decrypt(encText, context = "Unknown Key") {
    ...
    try {
        ...
        console.error(`[Crypto] Decrypt failed for '${context}': ${e.message}`);
        return null;
    }
}
